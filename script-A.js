  ...
  if (mediaQuery.matches) {
		...
		myDiv.innerHTML= `<div
                    ...
                    <div class="menu__page-top-a w-100 ">
						...
						<ul class="">
                    <li class="nav-item-a dropdown me-xxl-5 me-3 mb-lg-0 mb-4" id='ll'>
	...
	if (parentElement.contains(myDiv)) {
...
$(document).mouseup(function(e) {
	var container = $(".first__block-a");
	if (!container.is(e.target) && container.has(e.target).length === 0) {

	  	document.querySelector('.first-a').classList.add('hidden-block-a')
	}
  });
  ...
  function TrueEnjoyHintFirst(){
	document.querySelector('.dropdown-menu ').classList.add('show')
	document.querySelector('.dropdown-menu ').setAttribute('data-bs-popper', 'static')
	document.querySelector('.first-a').classList.add('hidden-block-a')
...
if (nv === null || nv === 'false') {
  ...
  var enjoyhint_instance = new EnjoyHint({
	...
	onStart:function(){
		document.querySelector('.first-a').classList.display = 'flex';
	  },
	onEnd:function(){
		document.querySelector('.dropdown-menu ').classList.remove('show')
		document.querySelector('.dropdown-menu ').removeAttribute('data-bs-popper', 'static')
	},
    showNext: true,
    nextButton: {className: "myNext", text: "Конечно!"},
    skipButton: {className: "mySkip", text: "Обойдусь"}
  });
  var enjoyhint_instance_mobile= new EnjoyHint({
	showNext: true,
	onStart:function(){
		disableMenuPageBlocks()
	  },
	onEnd:function(){
	console.log('end')
	},
	nextButtonMobile: {className: "myNextMobile", text: "Конечно!"},
    skipButtonMobile: {className: "mySkipMobile", text: "Обойдусь"}
  });

  // настраиваем шаги
  var enjoyhint_script_steps = [
	{ 'click .dropdown-menu ': 'Здесь вы можете сменить язык отоброжения интерфейса', showNext: true, 'nextButton': {className: "myNext", text: "Далее"}, 'skipButton': {className: "mySkip", text: "Пропустить"},'prevButton': {className: "myPrev", text: "Предыдущий"}},
	{ 'click #main': 'Тут вы можете узнать ваши активные и завершенные полисы', showNext: true, 'nextButton': {className: "myNext", text: "Далее"}, 'skipButton': {className: "mySkip", text: "Пропустить"},'prevButton': {className: "myPrev", text: "Предыдущий"}},
    { 'click #profile': 'Профиль это информация о вас по данным из купленного полиса', showNext: true, 'nextButton': {className: "myNext", text: "Далее"}, 'skipButton': {className: "mySkip", text: "Пропустить"}, 'prevButton': {className: "myPrev", text: "Предыдущий"}},
    { 'click #contacts': 'Вы можете оформить полис онлайн но так же есть возможность это сделать в нашем офисе. Так же имееться круглосуточный Call-Center', showNext: true, 'nextButton': {className: "myNext", text: "Далее"}, 'skipButton': {className: "mySkip", text: "Пропустить"}, 'prevButton': {className: "myPrev", text: "Предыдущий"}},
    { 'click #faq': 'Для быстрого ответа на ваш вопрос вы можете воспользоваться этим блоком или уточнить по номеру +7 727 320 0200', showNext: true, 'nextButton': {className: "myNext", text: "Далее"}, 'skipButton': {className: "mySkip", text: "Пропустить"}, 'prevButton': {className: "myPrev", text: "Предыдущий"} },
	{ 'click #setting': 'Если вы купили полис но в профиле изменения не отображаются то пожалуйста заполните эту форму и мы обновим всю информацию.  ', showNext: true, 'nextButton': {className: "myNext", text: "Далее"},  'prevButton': {className: "myPrev", text: "Предыдущий"} },
];
	

	var enjoyhint_script_steps_mobile = [
		{
			event: 'touchstart',
			selector: '.navbar-toggler',
			description: 'Это меню там вы можете сменить язык перейти в нужную вам страницу или заполнить форму. Нажмите на меню',
			showNext: true,
		...
		},
		{
			event: 'touchstart',
			selector: '#main',
			description: 'Тут вы можете узнать ваши активные и завершенные полисы',
			showNext: true,
			nextButton: {className: "myNext", text: "Далее"},
			skipButton: {className: "mySkip", text: "Пропустить"}
		},
		{
			event: 'touchstart',
			selector: '#profile',
			description: 'Профиль это информация о вас по данным из купленного полиса',
			showNext: true,
			nextButton: {className: "myNext", text: "Далее"},
			skipButton: {className: "mySkip", text: "Пропустить"},
			prevButton: {className: "myPrev", text: "Предыдущий"}
		},
		{
			event: 'touchstart',
			selector: '#contacts',
			description: 'Вы можете оформить полис онлайн но так же есть возможность это сделать в нашем офисе. Так же имееться круглосуточный Call-Center',
			showNext: true,
			nextButton: {className: "myNext", text: "Далее"},
			skipButton: {className: "mySkip", text: "Пропустить"},
			prevButton: {className: "myPrev", text: "Предыдущий"}
		},
		{
			event: 'touchstart',
			selector: '#faq',
			description: 'Для быстрого ответа на ваш вопрос вы можете воспользоваться этим блоком или уточнить по номеру +7 727 320 0200',
			showNext: true,
			nextButton: {className: "myNext", text: "Далее"},
			skipButton: {className: "mySkip", text: "Пропустить"},
			prevButton: {className: "myPrev", text: "Предыдущий"}
		},
		{
			event: 'touchstart',
			selector: '#setting',
			description: 'Если вы купили полис но в профиле изменения не отображаются то пожалуйста заполните эту форму и мы обновим всю информацию.',
			showNext: true,
			nextButton: {className: "myNext", text: "Далее"},
			prevButton: {className: "myPrev", text: "Предыдущий"}
		},
  ...
  enjoyhint_instance_mobile.set(enjoyhint_script_steps_mobile);
  // запускаем enjoyhint
  
  if (window.innerWidth > 900) {
	enjoyhint_instance.run();
  } else {
	console.log('enjoyhint запущен');
	enjoyhint_instance_mobile.run();
  ...
  function disableMenuPageBlocks() {
	console.log('disableMenuPageBlocks');
	const menuPageBlocks = document.querySelectorAll('.menu__page-block');
	menuPageBlocks.forEach((block) => {
	  block.disabled = true;
	});
  }
